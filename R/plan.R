...
the_plan <-
  drake_plan(
    ...
    # sim_varf_exp = function(nx, nfacet) {
    #   rep(dist_exponential(seq(rate, rate * nfacet, by = rate)), each = nx)
    # },
    
    sim_varf_gamma = function(nx, nfacet) {
      rep(dist_gamma(seq(shape, shape * nfacet, by = shape), rate), each = nx)
    },
    
    # sim_varf_weibull = function(nx, nfacet) {
    #   rep(dist_weibull(seq(shape, shape * nfacet, by = shape), scale), each = nx)
    # },
    
    # Design 3: Alternate where only distributions across x-varies
    
    sim_varx_normal = function(nx, nfacet) {
      rep(dist_normal(seq(mean, mean * nx, by = mean), sd), nfacet)
    },
    
    # sim_varx_exp = function(nx, nfacet) {
    #   rep(dist_exponential(seq(rate, rate * nx, by = rate)), nfacet)
    # },
    
    sim_varx_gamma = function(nx, nfacet) {
      rep(dist_gamma(seq(shape, shape * nx, by = shape), rate), nfacet)
    },
    
    # sim_varx_weibull = function(nx, nfacet) {
    #   rep(dist_weibull(seq(shape, shape * nx, by = shape), scale), nfacet)
    # },
    
    
    # Design 4: Alternate where all distribution varies
    
    sim_varall_normal = function(nx, nfacet) {
      dist_normal(seq(mean, mean * nfacet * nx, by = mean), sd)
    },
    
    # sim_varall_exp = function(nx, nfacet) {
    #   dist_exponential(seq(rate, rate * nfacet * nx, by = rate))
    # },
    
    sim_varall_gamma = function(nx, nfacet) {
      rep(dist_gamma(seq(shape, shape * nfacet * nx, by = shape), rate))
    },
    
    # sim_varall_weibull = function(nx, nfacet) {
    #   dist_weibull(seq(shape, shape * nfacet * nx, by = shape), scale)
    # },
    # 
