...
mpd2 <- function(sim_panel_data,
  ...
  panel_row <- distance_panel_data %>%
    ...
    unnest(c(value)) %>% 
    group_by(id_facet) %>% summarise(max = max(value)) %>% 
                         ...
                         function(i) {
                           ...
                           perm_data = sim_panel_adjust %>% select(-sim_data) %>%
                             ...
                             pivot_longer(-1, names_to = "dist",
                                          values_to = "perm_data") %>%
                             select(-dist) %>%
                             group_by(id_facet) %>% summarise(m = max(unlist(perm_data), na.rm = TRUE)) %>% summarise(perm_data = median(m, na.rm = TRUE))
                           #rename("perm_data" = "...2")
  ...
  stat <- nperm_data_n %>%
    summarise(mean = mean(perm_data, na.rm = TRUE),
